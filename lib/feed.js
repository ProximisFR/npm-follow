...
Feed.prototype.on_timeout = function on_timeout() {
  ...
  self.emit('timeout', {elapsed_ms:elapsed_ms, heartbeat:self.heartbeat, id:self.pending.request && self.pending.request.id()});
...
Feed.prototype.on_inactivity = function on_inactivity() {
  ...
  // prevent inactivty restart when feed is currently trying to reconnect
  // inactivty_timer will be setup once again when reconnect succeed
  if (self.retry_timer)
    return
  ...
  return self.restart("inactivity_restart");

}

Feed.prototype.restart = function restart(reason) {
  var self = this
